    ...
    const finalRoomData = rooms.map(room => {
      ...
      return {
        ...
        currentMeterReading: currentReading ? {
          water: currentReading.waterReading,
          electricity: currentReading.electricityReading,
        } : null,
        // 上次读数
        lastMeterReading: {
...
async function submitMeterReading(params, openId) {
  try {
    ...
    // 获取上个月的抄表记录作为上期读数
    const lastMonthDate = new Date(year, month - 2, 1) // month-2因为month从1开始
    ...
    try {
      ...
      if (lastMeterRecord.data.length > 0) {
        ...
        prevWaterReading = room.data.lastWaterReading || 0
        prevElectricityReading = room.data.lastElectricityReading || 0
      }
    } catch (error) {
    ...
    // 计算用量
    const waterUsage = Math.max(0, parseFloat(waterReading) - prevWaterReading)
    const electricityUsage = Math.max(0, parseFloat(electricityReading) - prevElectricityReading)
    
    // 检查是否已存在该月抄表记录
    const existingRecord = await db.collection('meter').where({
      roomId: roomId,
    ...
    if (existingRecord.data.length > 0) {
      // 更新现有记录
      result = await db.collection('meter')
        .doc(existingRecord.data[0]._id)
        .update({
          data: meterData
        })
      meterData.id = existingRecord.data[0]._id
    } else {
      // 创建新记录
      meterData.createdAt = new Date()
      result = await db.collection('meter').add({
        data: meterData
      })
    ...
    // 自动生成账单（如果房间已出租）
    let billResult = null
    if (room.data.status === 2) { // 已出租状态
      try {
        billResult = await generateBillFromMeter({
          roomId: roomId,
          readingMonth: readingMonth,
          meterData: meterData
        }, openId)
    ...
    return {
      ...
      data: {
        ...
        waterUsage,
        electricityUsage,
        billGenerated: billResult ? true : false,
